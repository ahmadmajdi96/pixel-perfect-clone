          ...
          <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-thin">
            {CAPA_STAGES.map((stage) => (
              <div key={stage} className="data-card min-w-[220px] flex-shrink-0">
                <h4 className="metric-label mb-3">
                  {CAPA_STAGE_LABELS[stage]}
                  <span className="ml-2 text-foreground">({capasByStage[stage]?.length ?? 0})</span>
                </h4>
                <div className="space-y-2">
                  {(capasByStage[stage] ?? []).map((capa: any) => (
                    <div
                      key={capa.id}
                      className="rounded-md border border-border p-2 cursor-pointer hover:border-primary/30 transition-colors"
                      onClick={() => navigate(`/capa/${capa.id}`)}
                    >
