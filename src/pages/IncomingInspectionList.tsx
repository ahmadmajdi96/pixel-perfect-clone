...
const IncomingInspectionList = () => {
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    if (status === "accepted" || status === "rejected" || status === "conditional") {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Incoming Goods Inspection</h1>
          <p className="metric-label mt-1">Inspect and disposition incoming materials</p>
        </div>
          ...
          <DialogContent>
            ...
            <form onSubmit={createInspection} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input value={form.ingredient} onChange={(e) => setForm({ ...form, ingredient: e.target.value })} required />
              </div>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">No inspections</TableCell></TableRow>
            ) : inspections.map((ins) => (
              <TableRow key={ins.id}>
                ...
                <TableCell>
                  <span className={`status-badge ${STATUS_COLORS[ins.status] ?? ""}`}>
